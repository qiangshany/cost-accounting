...
export async function GET(request: NextRequest) {
  try {
    ...
    // 构建查询条件
    let query = client.from('labor_maintenance_costs').select('*');
    ...
    const { data, error } = await query.order('cost_item_name', { ascending: true });
    ...
    console.error('查询人工与维护成本数据错误:', error);
    return NextResponse.json(
      {
        success: false,
...
export async function POST(request: NextRequest) {
  try {
    ...
    // { report_date, cost_item_name, product, workshop, amount, unit }
    ...
    return NextResponse.json({
      success: true,
      data: data || [],
      message: '人工与维护成本数据已成功提交',
    });
  } catch (error) {
    console.error('人工与维护成本提报错误:', error);
    return NextResponse.json(
      {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: '人工与维护成本数据已成功删除',
    });
  } catch (error) {
    console.error('删除人工与维护成本数据错误:', error);
    return NextResponse.json(
      {
